...
";
		...
		process.StartInfo.Arguments = $"/configure /cfg \"{tempInfPath}\" /overwrite";
...
";
	...
	private static void ConfigureUserAccounts()
	{
		...
		Console.WriteLine("Fetching all local users...");

		// Get all local users
		List<string> allLocalUsers = [];
		using (PrincipalContext ctx = new(ContextType.Machine))
		{
			UserPrincipal userPrincipal = new(ctx);
			PrincipalSearcher searcher = new(userPrincipal);
		...
		Console.WriteLine("Configuring standard users...");
		...
		Console.WriteLine("Looking at leftover users...");

		// Find users not specified by the administrator
		HashSet<string> specifiedUsers = new(adminUsers, StringComparer.OrdinalIgnoreCase);
